...
export class BestFiveFromSeven extends BaseGame {
    ...
    generateScenarios() {
        ...
        for (let i = 0; i < this.config.rounds; i++) {
            ...
            while (!scenario && attempts < 100) {
                ...
                if (bestHandResult.description.includes('High Card') && i > 3)
                    continue;
                scenario = {
                    id: `bf7-${i}`,
                    allCards: sevenCards,
    ...
    updateSelection() {
        ...
        allCards.forEach(cardEl => {
            const cardValue = cardEl.dataset.cardValue;
    ...
    submitSelection() {
        if (!this.currentScenario || this.selectedCards.size !== 5)
            return;
...
// Add styles
export function getBestFiveStyles() {
    return `
    .instructions {
      text-align: center;
      font-size: 1.2em;
...
//# sourceMappingURL=BestFiveFromSeven.js.map