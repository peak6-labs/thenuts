 ...
 */
export function getHandRankingValue(ranking) {
    return HAND_RANKINGS.indexOf(ranking);
}
/**
 * Compare two hand rankings
 */
export function compareHandRankings(a, b) {
 ...
 */
export function getThreeOfAKinds(cards) {
    const counts = countRanks(cards);
    const threes = [];
    for (const [rank, count] of counts) {
        if (count === 3)
 ...
 */
export function getFourOfAKinds(cards) {
    const counts = countRanks(cards);
    const fours = [];
    for (const [rank, count] of counts) {
        if (count === 4)
...
export function evaluateHand(cards) {
    ...
    if (isRoyalFlush)
        return { name: 'Royal Flush', rank: 10, cards: cardStrings };
    if (isStraightFlush(cards))
        return { name: 'Straight Flush', rank: 9, cards: cardStrings };
    const fours = getFourOfAKinds(cards);
    if (fours.length > 0)
        return { name: 'Four of a Kind', rank: 8, cards: cardStrings };
    const threes = getThreeOfAKinds(cards);
    const pairs = getPairs(cards);
    if (threes.length > 0 && pairs.length > 0)
        return { name: 'Full House', rank: 7, cards: cardStrings };
    if (isFlush(cards))
        return { name: 'Flush', rank: 6, cards: cardStrings };
    if (isStraight(cards))
        return { name: 'Straight', rank: 5, cards: cardStrings };
    if (threes.length > 0)
        return { name: 'Three of a Kind', rank: 4, cards: cardStrings };
    if (pairs.length >= 2)
        return { name: 'Two Pair', rank: 3, cards: cardStrings };
    if (pairs.length === 1)
        return { name: 'Pair', rank: 2, cards: cardStrings };
    return { name: 'High Card', rank: 1, cards: cardStrings };
}
/**
...
export function analyzeBoardTexture(communityCards) {
    ...
    return {
        ...
        possibleFlushes: Object.entries(suitCounts)
 ...
 */
function checkStraightPossibility(cards) {
    if (cards.length < 3)
        return false;
 ...
 */
function findPossibleStraights(cards) {
    const straights = [];
 ...
 */
export function getHandDescription(ranking, cards) {
    const parsedCards = cards.map(c => parseCard(c));
    switch (ranking) {
        case 'Royal Flush':
            return 'Royal Flush';
 ...
 */
export function getHandRank(evaluation) {
    return evaluation.rank;
}
/**
 * Generate a specific hand type for training games
 */
export function generateHandType(type, deck) {
    const shuffled = [...deck];
...
function findHandWithPairs(deck, pairCount) {
    const hand = [];
...
function findHandWithTrips(deck) {
